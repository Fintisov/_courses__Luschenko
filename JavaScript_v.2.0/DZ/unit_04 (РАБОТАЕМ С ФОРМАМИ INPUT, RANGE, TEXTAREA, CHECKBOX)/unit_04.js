...
function f4() {
    let a = document.querySelector('.i-4').checked;
...
// Task 5.
//Создайте input(checkbox).i-5 и button.b-5. Для checkbox добавьте value="task-5" - при нажатии на кнопку b-5
// выводите value checkbox если checkbox выбран и false если не выбран.
// Вывод везде в задачах, где не указано другое, осуществляется в div.out-номер задачи. В данном случае div.out-5
...
function f8() {
    out8.innerHTML = '<input class="i-81"> <button class="b-81 button-primary">Click</button>';
...
function f10() {
    ...
    let a = document.querySelector('.i-10');

    div.style.background = a.value;
...
// Task     11
//   Создайте input(color).i-111 и input(color).i-112- два элемента и button.b-11 -
...
function f15() {
    let a = document.querySelector('.i-15').value;//input
    let b = document.querySelector('.t-15');//textarea
...
// По нажатию кнопки выводите значение text и password в out-19 через пробел.
...
function f19() {
    ...
    //Думаю что кнопку мы вынесли за пределы для того, что бы кнопка не отправляла форму а точнее (password), и страница не перезагружалась.
}
...
//    div.innerHTML = form.elements[0].value + ' ' + form.elements[1].value;

//или
    div.innerHTML = form.elements['username'].value + ' ' + form.elements['password'].value;
}
