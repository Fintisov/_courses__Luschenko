...
Проверьте что происходит при повторном вызове функции.
...
function t3() {
    let out = "";
    let localArr = JSON.parse(localStorage.a2);
...
function t5() {
    let out = "";
    let localObj5 = JSON.parse(localStorage.a4)

    for (let key in localObj5) {
        out += `${key} ${localObj5[key]} <br>`;
    }
...
Создайте массив a7. Когда пользователь нажимает кнопку b-7 число должно добавляться в массив.
Массив должен сохраняться в LS с ключем a7.*/
let myArr7 = [];

function t7() {
    let result = document.querySelector(".i-7").value;
    myArr7.push(result);

    localStorage.setItem("a7", JSON.stringify(myArr7));
}
