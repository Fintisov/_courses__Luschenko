...
function f4() {
    div4.classList.toggle("bg-orange");
}
...
//  Task 5
// По нажатию .b-5 запускайте функцию f5, которая проверяет наличие класса .bg-orange у блока .out-4
// (да, именно у out-4 ).
...
function f5() {
    ...
    if (div4.classList.contains("bg-orange")) {
        out.innerHTML = true;
    } else {
        out.innerHTML = false;
    }
...
function f7() {
    let div = document.querySelectorAll(".out-7");
...
function f8() {
    let div = document.querySelectorAll(".out-8");
...
//  Task 10
// Усложним предыдущие задачи. С помощью цикла повecим на блоки .out-10 событие клик.
...
// Функция создает через createElement div c текстом 12 и добавляет ему класс bg-12.
...
//  Task 13
// Кнопка .b-13, запускает функцию f13.
// Функция создает через createElement div c текстом pushMe и добавляет ему класс bg-orange.
// Также, созданному div добавляется событие onclick, по которому выполняется функция f13_1.
...
//  Task 14
// Кнопка .b-14, запускает функцию f14.
// Функция создает через createElement div c текстом 14 и добавляет ему класс .bg-orange.
...
//  Task 15
// Кнопкa .b-15, которая запускает функцию f15.
// Функция создает через createElement div c текстом 15 и добавляет ему класс .bg-orange.
...
//  Task 16
// Кнопкa .b-16, которая запускает функцию f16.
// Функция создает через createElement div c текстом 16 и добавляет ему класс .bg-orange.
...
//  Task 17
// Кнопкa .b-17, которая запускает функцию f17.
// Функция создает через createElement div c текстом 17 и добавляет ему класс .bg-orange.
...
function f19() {
    ...
    for (let i = 0; i < p.length; i++) {
        battery += `${p[i].getAttribute("data-b")} `;
    }

    out.innerHTML = battery;
