...
const f6 = () => {
    let newSet = new Set(a6);
    document.querySelector(".out-6").innerHTML = newSet.size;
}
...
// Функция должна получать из i-7 значение пароля и проверять, чтобы пользователь в строке пароля использовал не повторяющиеся символы.
...
const f7 = () => {
    let i1 = String(document.querySelector(".i-7").value);
    let mySet = new Set(i1);
    let out = document.querySelector(".out-7");

    if (i1.length === mySet.size && mySet.size > 6) {
        out.textContent = 1;
    } else {
        out.textContent = 0;
    }
...
//  Функция должна принимать набор our_set в качестве параметра, преобразовывать его в строку,
//  причем после каждого символа строки должен быть пробел.
...
const f10 = (out_set, elem) => {
    return document.querySelector(elem).innerHTML = Array.from(out_set).join(" ")
}
