...
// Результат проверки - вывести в out-1. Проверьте на строке и set.
...
// В качестве критерия будем проверять следующее - содержимое переменной можно перебрать с помощью цикла for let i = .
// Т.е. все индексы должны быть числами. Функция должна выводить в out-2 true или false,
// в зависимости от того, что лежит в a2.
// Проверьте функцию на переменных указанных в комментариях.
...
// Обратите внимание на область применения reduce -
// везде где нужно в результате получить одно число на основе массива - reduce подойдет.
// Итак, в callback функции нам нужно сделать return accum + item.
// Результат - сумма элементов массива выводится в out-5.
...
// Обратите внимание на область применения reduce -
// везде где нужно в результате получить одно число на основе массива -
// reduce подойдет.
...
// Почему так?
// Разберитесь, а потом, после функции callback reduce добавьте 0.
...
const f9 = () => {
    let result = a9.reduce((accum, elem) => {
        if (elem > 0) {
            accum *= elem;
        }
        return accum;
...
// Функция должна перебрать массив a12 и вывести в out-12 индекс самого большого числа в массиве.
// Применяем reduce.
...
// Функция должна перебрать массив a13 и вывести в out-13 длину самого большого вложенного в a13 массива.
// Применяем reduce.
...
// Функция должна перебрать массив a14 и вывести в out-14 самый большой вложенный в a14 массив.
// Применяем reduce.
...
// Функция должна перебрать массив a15 измерений температуры за сутки
// и вывести в out-15 среднюю арифметическую температуру.
// Применяем reduce.
...
// Применяем reduce.
// Результирующий объект вывести на страницу в out-16 в формаете ключ пробел значение перенос строки.
...
// Результирующий массив должен находится в a17_res.
...
const f17 = () => {
    ...
    document.querySelector(".out-17").innerText = a17_res.join(" ");
}
...
// Для формирования строки для вывода используйте reduce.
// Чтобы из строки сделать массив - split.
// Результат вывести в out-19.
...
const f20 = () => {
    a20 = a20.reverse();
    a20 = a20.join(" ");
    document.querySelector(".out-20").textContent = a20;
}
